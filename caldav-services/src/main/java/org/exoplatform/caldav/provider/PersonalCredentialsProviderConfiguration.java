 ...
 * {@link ConditionalOnMissingBean} rather than owned outright by this add-on: the
 * email connector declares the exact same bean method in its own module, so the
 * provider's existence never depends on any one specific Personal-mode connector
 * being installed - only on at least one of them being present.
 */
@Configuration
