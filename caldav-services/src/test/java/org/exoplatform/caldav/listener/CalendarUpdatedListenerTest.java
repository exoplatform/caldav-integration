...
public class CalendarUpdatedListenerTest {
  ...
  private CalendarUpdatedListener listener;
  ...
  public void wireTheEngine() {
    listener = new CalendarUpdatedListener();
   ...
   */
  @Test
  public void aRenamedCalendarIsCarriedOverWithoutWaitingForTheThrottle() {
    listener.onEvent(eventFor(calendar(OWNER)));

    verify(caldavSyncService).syncAfterCalendarUpdated(OWNER);
  }
  ...
  public void aCalendarWithNoOwnerIsIgnored() {
    ...
    verify(caldavSyncService, never()).syncAfterCalendarUpdated(anyLong());
  }
   ...
   */
  @Test
  public void aFailingSyncDoesNotUndoTheCalendarEdit() {
    ...
    doThrow(new IllegalStateException("down")).when(caldavSyncService).syncAfterCalendarUpdated(OWNER);
  ...
  private Event<Calendar, Object> eventFor(Calendar calendar) {
    return new Event<>("exo.agenda.calendar.updated", calendar, null);
  }
}
