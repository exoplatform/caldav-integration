...
# The password reaches curl on STDIN (--data-binary @-), never as an argument
# to python3 and never as one to curl: an argv is visible in ps to every local
# user for the life of the process, and curl's life here is the whole HTTP
# call. The header above promises the password is never passed as an argument;
# this is the only step that sends it as a body, and it is the step that has to
# honour that promise the way capture() does with --config.
status=$(printf '%s' "$PASSWORD" \
  | python3 -c 'import json,sys; sys.stdout.write(json.dumps(sys.stdin.read()))' \
  | curl -s -X POST -H 'Content-Type: application/json' -H 'Accept: application/json' --data-binary @- -D "$tmp_headers" -o "$tmp_body" -w '%{http_code}' "$LOGIN_URL" || echo 'unreachable')
